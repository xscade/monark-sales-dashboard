...
import { AlarmClock, CalendarPlus, Check, CircleAlert, CircleCheck, X } from "lucide-react";
...
export default async function FollowUpsPage({
  ...
  const presets = buildReschedulePresets(new Date(), user.timezone);
  const scopeCounts = { due: counts.due, missing: counts.missing, all: counts.due + counts.missing };
  ...
  return (
    <div className="space-y-5">
          ...
          <p className="text-sm text-zinc-500">
            {counts.overdue > 0 ? (
              <span className="font-semibold text-red-600 dark:text-red-400">
                {formatNumber(counts.overdue)} overdue
              </span>
            ) : (
              "Nothing overdue"
            )}
            {" · "}
            {formatNumber(counts.due)} scheduled · {formatNumber(counts.missing)} with no next step
          </p>
          <p className="mt-0.5 text-xs text-zinc-400">
            Who is going cold, and in what order to call them.
          </p>
        </div>
      ...
      {/* Dismissible, because the flash lives in the URL: without a way out it
          survives every sort, filter and page change the person makes next. */}
      {params.notice && (
        <div className="flex items-center gap-2 rounded-lg border border-emerald-300 bg-emerald-50 px-3 py-2 text-sm text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-300">
          <CircleCheck aria-hidden className="size-4 shrink-0" />
          <span className="min-w-0 flex-1">{params.notice}</span>
          <Link href={returnTo} aria-label="Dismiss message" className="rounded p-0.5 opacity-70 transition hover:opacity-100">
            <X aria-hidden className="size-4" />
          </Link>
        </div>
      )}
      {params.error && (
        <div className="flex items-center gap-2 rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/40 dark:text-red-300">
        ...
        {FOLLOW_UP_SCOPES.map((option) => {
          ...
          return (
            ...
            >
              ...
              {/* The count belongs on the filter itself — otherwise picking one
                  is a guess about whether there is anything behind it. */}
              <span
                className={`tabular rounded px-1.5 text-xs ${
                  active ? "bg-white/20" : "bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400"
                }`}
              >
                {formatNumber(scopeCounts[option])}
              </span>
            </Link>
          );
        })}
      ...
      {rows.length === 0 ? (
        ...
        <div className="space-y-2.5">
          {rows.map((row) => {
            const callTarget = buildCallTarget(row.primaryPhone);
            return (
              <Card key={row.leadId} className={row.isOverdue ? "border-red-300 dark:border-red-900/70" : ""}>
                <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="flex shrink-0 flex-wrap items-start gap-2">
                    {callTarget && (
                      <CallButton target={callTarget} name={row.fullName ?? row.reference} />
                    )}
                    {!mayWrite ? null : row.taskId ? (
                      <>
                        <RescheduleFollowUp
                          ...
                          returnTo={returnTo}
                          presets={presets}
                          currentDueLabel={row.dueAt ? formatDateTime(row.dueAt) : null}
                          leadName={row.fullName ?? row.reference}
                        />
