...
export default async function NewWalkInPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Reception desk</p><h1 className="mt-1 text-2xl font-bold tracking-tight">New walk-in</h1><p className="mt-1 text-sm text-muted-foreground">Capture the customer, the opportunity and — when they are here — the arrival, in one transaction.</p></div><span className="inline-flex items-center gap-1.5 self-start rounded-full border bg-card px-3 py-1 text-xs text-muted-foreground"><WifiOff className="size-3.5" />Offline queue is ready</span></div>
